...
const ApplyPage = async ({ params }) => {
    ...
    // ✅ Session-এর বদলে DB থেকে fresh user data আনো
    const BASE_URL = process.env.NEXT_PUBLIC_SERVER_URL;
    const freshUserRes = await fetch(`${BASE_URL}/api/users/me?email=${sessionUser.email}`, {
        cache: 'no-store'
    });
    const freshUser = await freshUserRes.json();

    console.log("freshUser:", freshUser);
console.log("planId:", freshUser?.plan);

    // fresh user থেকে plan নাও, না থাকলে seeker_free
    const planId = freshUser?.plan || 'seeker_free';

    const applications = await getApplicationsByApplicant(sessionUser.id);

    let plan = await getPlanById(planId);
console.log("plan from DB:", plan);
    if (!plan) {
        plan = { id: 'seeker_free', name: 'Free', maxApplicationsPerMonth: 3 };
    }
        ...
        <div className="min-h-screen bg-black text-white py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="p-6 rounded-2xl bg-zinc-900 border border-zinc-800 shadow-lg space-y-4">
                    ...
                    <div className="w-full bg-zinc-800 h-2 rounded-full overflow-hidden">
                        <div
                            className={`h-full transition-all duration-500 ${isLimitExceeded ? 'bg-rose-500' : 'bg-emerald-500'}`}
                            style={{ width: `${Math.min((applications.length / plan.maxApplicationsPerMonth) * 100, 100)}%` }}
                        />
                    </div>
                </div>
                ...
                {isLimitExceeded ? (
                    <div className="p-6 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-center space-y-3">
                        ...
                        <h3 className="text-lg font-bold text-rose-400">Monthly Application Limit Reached</h3>
                        <p className="text-zinc-400 text-sm max-w-md mx-auto leading-relaxed">
                            You have used all {plan.maxApplicationsPerMonth} applications for this month. Upgrade your plan to keep applying.
                        </p>
                        <div className="pt-2">
                            <Link
                                href="/allRoute/pricing"
                    ...
                    <div className="bg-zinc-950 p-6 rounded-2xl border border-zinc-900">
                        <JopApply applicant={sessionUser} job={job} />
                    </div>
                )}
